...
//   المشكلة: wifidog مش بيدعم TLS — بيبعت HTTP عادي على بورت 443
//   فالراوتر مش قادر يتكلم مع Vercel (HTTPS only)
//   الحل: socat relay محلي — wifidog → 127.0.0.1:8081 → TLS+SNI → السيرفر
// ═══════════════════════════════════════════════════════════
...
function buildFixScript(opts: {
  ...
  const clientTimeout = parseInt(String(opts.clientTimeout || '10'), 10)
...
#   والسيرفر على Vercel بيقبل HTTPS فقط
...
# (ده اللي بيخلي wifidog يبعت Host الصحيح مع راوتر التحويل)
# ────────────────────────────────────────────────
...
echo ""
echo " 🔥 جرب دلوقتي:"
echo "    1- اتصل بشبكة الواي فاي"
echo "    2- افتح أي موقع → هتتحول لصفحة الدخول"
echo "    3- اكتب رقم الكارت → المفروض يشتغل النت فوراً"
echo "=============================================="
echo ""
`
}
...
export async function GET(req: NextRequest) {
  ...
  let realIps: string[] = []
  try {
    realIps = await dns.resolve4(authHost)
  } catch {
    try {
      realIps = await dns.resolve4('cname.vercel-dns.com')
    } catch {}
  }
